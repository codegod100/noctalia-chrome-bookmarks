...
echo "🚀 Installing Quick Notes plugin for Noctalia..."
...
if [ -f "$NOCTALIA_PLUGINS_JSON" ]; then
    ...
    if grep -q "\"$PLUGIN_NAME\"" "$NOCTALIA_PLUGINS_JSON"; then
        ...
        # Add plugin entry (simple append, may need manual cleanup for proper JSON)
...
echo "  3. Open launcher (Super+D) and type >note"
echo ""
echo "For keyboard shortcut, add to your compositor config:"
echo "  noctalia-shell ipc call plugin:quick-notes toggle"
echo ""
